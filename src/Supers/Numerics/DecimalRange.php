...
class DecimalRange extends Decimal
{
    ...
    protected function _predicate($input, null|string|array &$message = null): bool
    {
        ...
        if ($this->min !== null and $input < $this->min) {
            $message = ExpressionService::getXua('supers.numerics.decimal_range.error_message.value_must_be_at_least_min', [
                'value' => $input,
                'min' => $this->min
            ]);
            return false;
        ...
        if ($this->max !== null and $input > $this->max) {
            $message = ExpressionService::getXua('supers.numerics.decimal_range.error_message.value_must_be_at_most_max', [
                'value' => $input,
                'max' => $this->max
            ]);
            return false;
